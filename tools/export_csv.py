...
Writes two files: the clubs CSV, and a feedback CSV next to it
(default: feedback.csv in the same directory) with one row per
feedback entry — platform summaries from club_feedback and local-diver
quotes from diver_quotes (issue #17).

Usage:
    python tools/export_csv.py public/dive_clubs.db data/clubs.csv
    python tools/export_csv.py public/dive_clubs.db data/clubs.csv --feedback-csv data/feedback.csv
"""
...
import db
from schema import CSV_COLUMNS, FEEDBACK_CSV_COLUMNS
...
def export_feedback_rows(conn):
    """One CSV row per feedback entry: platform rows from club_feedback,
    local-diver rows from diver_quotes, ordered by club then source so the
    file diffs stably.
    """
    rows = []
    ...
    platform = conn.execute(
        """
        SELECT f.club_id, fs.source_name, fs.source_kind, f.rating,
               f.review_count, f.url, f.summary, f.lang, f.last_checked
          FROM club_feedback f
          JOIN feedback_sources fs ON fs.source_id = f.source_id
         ORDER BY f.club_id, fs.source_name
        """
    ).fetchall()
    for r in platform:
        rows.append({
    ...
    quotes = conn.execute(
        "SELECT club_id, quote, author_alias, quoted_at, lang FROM diver_quotes ORDER BY club_id, quote_id"
    ).fetchall()
    for r in quotes:
        rows.append({
            "club_id": r["club_id"],
    ...
    rows.sort(key=lambda r: (r["club_id"], r["kind"], r["source"]))
    return rows
...
def main():
    ...
    parser.add_argument("csv_path", help="Path to the output clubs CSV file (overwritten)")
    parser.add_argument("--feedback-csv", help="Path to the output feedback CSV (default: feedback.csv next to the clubs CSV)")
    ...
    feedback_rows = export_feedback_rows(conn)
    conn.close()
    ...
    with open(feedback_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=FEEDBACK_CSV_COLUMNS)
        writer.writeheader()
        writer.writerows(feedback_rows)

    print(f"Wrote {len(rows)} club(s) to {args.csv_path}")
    print(f"Wrote {len(feedback_rows)} feedback row(s) to {feedback_path}")
